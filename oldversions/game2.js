...
//const QUALITIES = ['Faith', 'Courage', 'Knowledge', 'Love', 'Wisdom', 'Healing', 'Joy', 'Focus', 'Prosperity', 'Purity', 'Humility', 'Forgiveness', 'Hope', 'Praise', 'Intercession', 'Endurance', 'Good News', 'Identity'];
// these QUALITIES soon should be taken from get_verses
let QUALITIES;
let vQuality;
if (gameCategory != '')  vQuality = gameCategory;
let qualityButtons = [];
let updateButtonsTimer = null;
let gameLevel = 1;
let levelCompleted = false;

let urlBase = 'https://credtec.co/g1/';
...
let gameState = 'game'; // Possible values: 'game', 'review'
...
async function loadVerses() {
    ...
    try {
        const response = await fetch(urlBase + 'get_verses.php?category=' + gameCategory);
        if (!response.ok) {
            throw new Error('Network response was not ok ' + response.statusText);
        }
        const data = await response.json();
        console.log('Verses loaded:', data);
        organizedVerses = data; // Assign the retrieved verses to organizedVerses
    } catch (error) {
...
async function init() {
    ...
    try {
        ...
        QUALITIES = Object.keys(organizedVerses);
    ...
    //verses = loadSelectedVerses(); user
    console.log("Hopefully loaded from database:", organizedVerses);
    organizedVerses = organizeByCategory2(organizedVerses);
    ...
    canvas.height = Math.min(700, window.innerHeight - 30); // Set the canvas height to 700 pixels (for Samsung Galaxy A53 in portrait mode)
    ...
    if (gameState === 'game') {
        ...
        monsters.forEach(monster => {
            if (monster.chaser) {
                ...
                if (Date.now() - monster.behaviorStartTime > 30000) {
                    if (gameLevel === 2 && Math.random() < 0.5) {
        ...
        monsters.forEach(monster => {
            ...
            if (distance < COMBAT_DISTANCE) {
                ...
                if (currentTime - lastAttackTime > ATTACK_RATE) {
                    ...
                    if (isAnswerCorrect === true) {
                        monster.health -= 2; // Player attacks the monster
                    ...
                    const damage = Math.floor(Math.random() * (monster.maxDamage + 1) * gameLevel);
            ...
            // Prevent player from getting too close to the monster
            /*
            if (distance < MINIMUM_DISTANCE) {
              let angle = Math.atan2(dy, dx);
              let newPlayerX = player.x - Math.cos(angle) * MINIMUM_DISTANCE;
              let newPlayerY = player.y - Math.sin(angle) * MINIMUM_DISTANCE;

              // Check if the new player position is within the canvas boundaries
              if (
                newPlayerX >= 0 &&
                newPlayerX <= canvas.width - player.width &&
                newPlayerY >= QUALITY_LINE_HEIGHT + BUTTON_HEIGHT &&
                newPlayerY <= canvas.height - player.height - ANSWER_SECTION_HEIGHT
              ) {
                player.x = newPlayerX;
                player.y = newPlayerY;
              }
        ...
        if (monsters.length === 0 && spawnsLeft === 0 && !levelCompleted) {
            ...
            setTimeout(() => {
                maxSpawns += 2;
                spawnsLeft = maxSpawns;
                gameLevel++; // Increment the game level
...
function spawnMonster() {
    if (monsters.length < MAX_MONSTERS && spawnsLeft > 0) {
        let x, y;
        do {
            x = Math.random() * (canvas.width - monsterImg.width);
            y = Math.random() * (canvas.height - monsterImg.height - ANSWER_SECTION_HEIGHT - QUALITY_LINE_HEIGHT - BUTTON_HEIGHT) + QUALITY_LINE_HEIGHT + BUTTON_HEIGHT;
        } while (isOverlapping(x, y, monsterImg.width, monsterImg.height));
        ...
        const demonType = demonTypes[Math.floor(Math.random() * demonTypes.length)];
        ...
        monsters.push({
            ...
            y: y,
            health: 10,
            width: monsterImg.width,
            height: monsterImg.height,
            demonType: demonType, // Assign the randomly selected demon type
            maxDamage: maxDamage, // Assign the maximum damage based on the demon type
            chaser: chaser, // Flag to indicate if the monster should chase the player
            chasingStartTime: null, // Add a new property to track the chasing start time
            behaviorStartTime: Date.now(),
            showHealth: true, // Flag to show monster's health
            showHealthTimeout: null, // Timeout reference for health display
            isAttacked: false,
            healthBar: { // New property to represent the health bar
                x: 0,
                y: 0,
                width: 0,
                height: 7, // Scale the health bar height
                color: 'green' // Set the desired color of the health bar
            },
            ...
            randomWalk: function() {
                ...
                if (this.walkingDistance === undefined) {
                    this.walkingDistance = Math.random() * (MAX_WALK_DISTANCE - MIN_WALK_DISTANCE) + MIN_WALK_DISTANCE;
                    this.angle = Math.random() * 2 * Math.PI;
                }

                let dx = Math.cos(this.angle) * MONSTER_SPEED;
                let dy = Math.sin(this.angle) * MONSTER_SPEED;
                this.x += dx;
                this.y += dy;
                this.walkingDistance -= MONSTER_SPEED;

                // Check for wall collisions
                if (this.x > canvas.width - this.width || this.x < this.width) { // changed from this.width /2 whole monsters only on left
                    this.x -= dx;
                }
                if (this.y > canvas.height - this.height / 2 - ANSWER_SECTION_HEIGHT || this.y < this.height / 2 + QUALITY_LINE_HEIGHT + BUTTON_HEIGHT) {
                    this.y -= dy;
                }

                // Reset the walking distance and angle if the current walk is finished
                if (this.walkingDistance <= 0) {
                    this.walkingDistance = undefined;
...
function startVerseAudio(verseReference) {
    ...
    playAudio($vdir + ".ogg")
        .then(() => {
            ...
            if (repeatEnabled && gameState === 'review' && verseReference === getCurrentVerseReference()) {
                setTimeout(() => {
                    if (repeatEnabled && gameState === 'review' && verseReference === getCurrentVerseReference()) {
                        startVerseAudio(verseReference);
                    }
